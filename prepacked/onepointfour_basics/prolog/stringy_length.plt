...
test("accept length and type of string") :-
   stringy_length_type("hello",5,string).

test("reject length and type of string due to type mismatch", fail) :-
   stringy_length_type("hello",5,atom).

test("reject length and type of string due to length mismatch", fail) :-
   stringy_length_type("hello",55,string).

test("failure because of negative length given", fail) :-
   stringy_length_type("hello",-1,string).

test("failure because of bad type", fail) :-
   stringy_length_type("hello",5,nonesuch).

test("exception because of negative length given", error(check(domain,_,_,_))) :-
   stringy_length_type("hello",-1,string,hard).

test("exception because of bad type", error(check(domain,_,_,_))) :-
   stringy_length_type("hello",5,nonesuch,hard).

test("determine length and type") :-
   stringy_length_type("hello",L,T,hard),
   assertion(L==5),
   assertion(T==string).
